...
window.sjm.state.play = {
  ...
  create: function(){
    ...
    //创建初始变量
    this.stepCount=0;
    this.name=['终极摸金校尉','超级摸金校尉','高级摸金校尉','摸金校尉'];//随便定的4个等级

    gameData.track('财神-游戏开始');
  },
  createMap:function(){
    for(var i=0;i<64;i++){
      //随机定不可称动区域
  ...
  gameOver:function(bz){//游戏结束了
    ...
    if(bz){//捉到猫了
      ...
      if (this.stepCount < 100) {
        this.gift=mt.create('gift');
        this.gift.inputEnabled=true;
        this.gift.events.onInputDown.add(this.giftfn,this);
        this.replay.y -= 80;
      }
      ...
      // 没抓到，设置按钮位置
      this.replay.x = this.world.width / 2
      this.replay.anchor.x = 0.5
      this.replay.anchor.y = 0.5
      this.code=mt.create('showcode');
      this.code.x = this.world.width / 2
      this.code.anchor.x = 0.5
      this.code.anchor.y = 0.5
      this.code.inputEnabled=true;
      this.code.events.onInputDown.add(this.showErcodeFn, this)
    }
  ...
  giftfn: function () {//寻找礼物
    console.log('财神-点击我要学习')
    // 统计
    gameData.track('财神-点击我要学习');

    if (debug) {
      // 测试连接
      // 21天报名
      window.location.href = 'https://h5test.ichangtou.com/minic/index21Intro.html?Game=Game'
    } else {
      // 正式链接
      // 21天报名 3元
      window.location.href = 'https://h5.ichangtou.com/minic/index21Intro.html?Game=Game'
    }
  },
   ...
   * 显示二维码
   */
  showErcodeFn: function () {
    gameData.track('财神-显示二维码');
    ...
    const currPic = this.victory || this.failed
    currPic.destroy()
    // this.replay.destroy()

    // // 显示二维码
    // const currPic = this.victory || this.failed
    // this.QRcode = mt.create('ercode');
    //
    // // 二维码居中
    // this.QRcode.anchor.x = 0.5
    // this.QRcode.anchor.y = 0.5
    // this.QRcode.x = this.world.width / 2

    // 二维码文案
    const bannerText = '长按二维码关注我们'
    let banner = this.add.text(this.world.centerX, this.game.height - 380, bannerText)
    banner.fill = '#fff'
